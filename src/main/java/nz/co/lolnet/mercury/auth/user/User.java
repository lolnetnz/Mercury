...
public class User {
	...
	public boolean checkCredentials(String token) {
		if (userData == null) {
			this.error = new JsonResponse().error("Not Found", "The server was unable to find a user matching the supplied uuid.");
			return false;
		}
		...
		this.error = new JsonResponse().error("InternalServerError", "The server was unable to verify the users credentials.");
		return false;
	}
		...
		if (permissions != null) {	
			for (String permission : permissions) {
				if (!userData.getPermissions().contains(permission)) {
					this.error = new JsonResponse().error("Forbidden", "The user doesn't not have the necessary permissions to complete this request.");
					return false;
				}
			}
			return true;
		}
		this.error = new JsonResponse().error("InternalServerError", "The server was unable to verify the users permissions.");
		return false;
	}
